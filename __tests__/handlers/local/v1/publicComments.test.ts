...
import { publicComments } from "@/handlers/local/v1/publicComments";
...
describe("publicComments", () => {
  it("returns paginated comments and summary", async () => {
    const result = await publicComments("public-council-1", "APP-123", {
      page: 1,
      resultsPerPage: 2,
      type: "public",
    });
  ...
  it("returns no comments if reference is APP-NULL", async () => {
    const result = await publicComments("public-council-1", "APP-NULL", {
      page: 1,
      resultsPerPage: 2,
      type: "public",
    });
