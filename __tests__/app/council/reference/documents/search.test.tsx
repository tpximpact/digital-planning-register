 ...
 */
import { GET } from "@/app/[council]/[reference]/documents/search/route";
...
jest.mock("@/config", () => ({
  getAppConfig: jest.fn(() => ({
    features: {
      documentSearchFields: ["field1", "field2"],
    },
  })),
}));
...
const getMockRequest = (params: Record<string, string>) => {
  ...
  return {
    nextUrl: {
      searchParams,
      pathname: "/council/reference/documents/search",
    },
...
describe("GET route", () => {
  ...
  it("filters search params if action is 'clear'", async () => {
    ...
    expect(redirect).toHaveBeenCalledWith(
      expect.stringMatching(/^\/council\/reference\/documents\?/),
    );
  });
  ...
  it("removes undefined values from query params", async () => {
    ...
    const { validateSearchParams } = require("@/lib/documents");
