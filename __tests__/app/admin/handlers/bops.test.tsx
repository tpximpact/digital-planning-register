...
import Main from "@/app/(private)/admin/handlers/bops/page";
...
describe("BOPS Handler Main Page", () => {
  it("renders the title, description, and security warning", () => {
    render(<Main />);
    expect(
      screen.getByRole("heading", { name: "BOPS handler" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/This page documents the BOPS handler/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/will not work outside of a development environment/i),
    ).toBeInTheDocument();
  });

  it("renders an EndpointCard for each BopsV2 endpoint", () => {
    render(<Main />);
