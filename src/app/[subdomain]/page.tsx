...
export default function Home() {
  ...
  useEffect(() => {
    ...
    (async() => {
      ...
      const response = await getApplicationsByCouncil(council, 1, resultsPerPage);
...
async function handlePageClick(event: any) {
  const response = await getApplicationsByCouncil(council, event.selected + 1, resultsPerPage);
      setData(response.data);
}
