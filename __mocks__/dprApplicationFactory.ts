...
export const generateDocument = (): DprDocument => {
  ...
  const submittedAt = faker.date.anytime().toISOString();
  const validatedAt = faker.date.anytime().toISOString();
  const publishedAt = faker.date.anytime().toISOString();
  return {
    id: faker.number.int({ min: 1, max: 10000 }),
    name: faker.lorem.lines(1),
    association: "application" as PostSubmissionFileAssociation,
    // association: faker.helpers.arrayElement([
    //   "application",
    //   "appeal",
    //   "specialistComment",
    //   "publicComment",
    // ]) as PostSubmissionFileAssociation,
    // version: faker.number.int({ min: 1, max: 10 }),
    type: faker.helpers.arrayElements<FileType>(
      [
